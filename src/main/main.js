...
ipcMain.handle('open-vendor-folder', () => {
  ...
  if (fs.existsSync(binPath)) shell.openPath(binPath);
});
...
ipcMain.on('download-video', (e, { downloadOptions, jobId }) => {
  ...
  proc.on('error', (spawnError) => {
    ...
    if (win)
      win.webContents.send('download-info-error', {
        jobId,
        error: `Failed to start download process: ${spawnError.message}`,
        fullLog: spawnError.message,
      });
  });
  ...
  for (let i = 0; i < filePaths.length; i++) {
    ...
    try {
      const { stdout } = await new Promise((res) => {
        ...
        p.on('error', (e) => {
          console.error('Import spawn error:', e);
          res({ stdout: '' });
        });
      });
